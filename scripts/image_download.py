...
def download_geostationary_imgs(dates, save_path, lon_range, lat_range,
    ...
    for date in dates:
        ...
        url = (f'https://wvs.earthdata.nasa.gov/api/v1/snapshot?REQUEST=GetSnapshot&TIME={date_str}&BBOX={lat1},{lon1},{lat2},{lon2}&CRS=EPSG:4326&LAYERS={var}&FORMAT=image/jpeg{size}')
        ...
        #        size)
        print(url)
        save_str = (save_path+f'/{satellite}_'+var +
            date.strftime('%Y%m%d_%H%M%S') + loc_str + '.' + filetype)
    ...
    for yr in range(year_range[0], year_range[1]):
        for m in months:
            ...
            for nd in range(1,nday+1):
                ...
                url = (f'https://wvs.earthdata.nasa.gov/api/v1/snapshot?REQUEST=GetSnapshot&TIME={yr}-{m:02g}-{nd}&BBOX={lat1},{lon1},{lat2},{lon2}&CRS=EPSG:4326&LAYERS={var}&FORMAT=image/jpeg{size}')
                # url = ('https://gibs.earthdata.nasa.gov/image-download?TIME='+
                #        str(yr)+d+loc+'&epsg=4326'+layer+
                #        '&opacities=1,1&worldfile=false&format=image/'+filetype+
                #        size)
